...
vector::vector( const vector &right )
   : myFirst(),
     ...
     myEnd()
{
    value_type size = right.myLast - right.myFirst;
    value_type* str = new value_type[size]();
    myFirst = str;
    myLast = myEnd = str + size;
...
vector& vector::assign( const vector &right )
{
   ...
   {
       size_t right_size = right.myLast - right.myFirst;
       size_t right_capacity = right.myEnd - right.myFirst;
       ...
       {
           size_t add = ((capacity() * 1.5) < right_size) ? right_size : capacity() * 1.5;
           value_type* str = new value_type[add]();
           for (size_t i = 0; i < right_size; i++)
               str[i] = right.myFirst[i];
           myFirst = str;
           myLast = str + right_size;
           myEnd = str + add;
       }
   }
...
void vector::resize( const size_type newSize )
{
    ...
    {
        size_t add = ((capacity() * 1.5) <= newSize) ? newSize : capacity() * 1.5;
        value_type* str = new value_type[add]();
