...
bool HugeInteger< T >::operator<(HugeInteger< T >& right) {
    ...
    auto l = --integer.end();
    auto r = --right.integer.end();
    for (int i = 0; i < integer.size(); ++i, --l, --r)
        if (*l != *r) return (*l < *r);
    return false;
} // end function operator<
...
HugeInteger< T > HugeInteger< T >::operator-(HugeInteger< T >& op2) {
    ...
    auto i = difference.integer.begin(), j = op2.integer.begin();
    for (; j != op2.integer.end(); ++i, ++j) {
        if (*i < *j) {
            auto temp = i;
            for (++temp; *temp == 0; ++temp) *temp = 9;
            *i += 10;
            --(*temp);
        }
        *i -= *j;
    ...
    while (difference.integer.size() > 1 && *difference.integer.rbegin() == 0) 
...
HugeInteger< T > HugeInteger< T >::operator*(HugeInteger< T >& op2) {
    ...
    for (auto it1 = integer.begin(), it2 = product.integer.begin(); it1 != integer.end(); ++it1, ++it2)
    ...
    while (product.integer.size() > 1 && *product.integer.rbegin() == 0)
...
HugeInteger< T > HugeInteger< T >::operator/(HugeInteger< T >& op2) {
    ...
    auto quo = --quotient.integer.end();
    for (int k = quotient.integer.size() - 1; k >= 0; --k, --quo) {
        *quo = 10;
        do {
            --(*quo);
            buffer = op2 * quotient;
        } while (*quo > 0 && *this < buffer);
    }

    while (quotient.integer.size() > 1 && *quotient.integer.rbegin() == 0)
