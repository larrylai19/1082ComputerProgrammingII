...
public:
    ...
    list& operator=(const list& right) {
        if (this != &right) {
            if (!myData.myHead) {
                myData.myHead = new node{ nullptr , nullptr ,Ty() };
                myData.myHead->prev = myData.myHead->next = myData.myHead;
            }
    ...
    void push_front(const Ty& val) {
        ++myData.mySize;
        myData.myHead->next = new node{ myData.myHead->next, myData.myHead, val };
        myData.myHead->next->next->prev = myData.myHead->next;
    }
    ...
    void push_back(const Ty& val) {
        ++myData.mySize;
        myData.myHead->prev = new node{ myData.myHead, myData.myHead->prev, val };
        myData.myHead->prev->prev->next = myData.myHead->prev;
    }
...
bool operator==(const list< Ty >& left, const list< Ty >& right) {
    ...
    for (auto i = left.begin(), j = right.begin(); j != right.end(); ++i, ++j)
        if (*i != *j) return false;
    return true;
}
