...
public:
   ...
   deque& operator=( const deque &right )
   {
      if( &right != this ) // avoid self-assignment
      {
          if (myData.mapSize < right.myData.mapSize)
          {
          ...
          for (size_type i = 0; i < right.myData.mapSize; ++i)
          {
              //if (myData.map[i] != nullptr && right.myData.map[i] == nullptr)
              //    delete[] myData.map[i];
              if (right.myData.map[i] != nullptr && myData.map[i] == nullptr)
                  myData.map[i] = new value_type[4]();
              if (right.myData.map[i] != nullptr)
                  for (int j = 0; j < 4; ++j)
                      myData.map[i][j] = right.myData.map[i][j];
...
//      return ( ( myData.off / 4 ) & ( myData.mapSize - 1 ) );
   ...
   void doubleMapSize()
   {
      if( myData.mapSize > 0 )
      {
          Ty** newMap = new Ty * [myData.mapSize * 2]();
