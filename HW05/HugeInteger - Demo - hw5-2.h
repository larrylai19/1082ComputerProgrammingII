...
#include "Vector - Student ID - hw5.h" // include definition of class vector 
...
HugeInteger< T > HugeInteger< T >::operator-(HugeInteger< T >& op2) {
    ...
    for (size_t i = 0, j = 0; i < op2.integer.size(); ++i, j = i) {
        if (difference.integer[i] < op2.integer[i]) {
            while (difference.integer[++j] == 0) difference.integer[j] = 9;
            --difference.integer[j];
            difference.integer[i] += 10;
        }
        difference.integer[i] -= op2.integer[i];
    }

    while (difference.integer[difference.integer.size() - 1] == 0) difference.integer.pop_back();
...
HugeInteger< T > HugeInteger< T >::operator*(HugeInteger< T >& op2) {
    ...
    for (size_t i = 0; i < integer.size(); ++i)
        for (size_t j = 0; j < op2.integer.size(); ++j) {
            ...
            product.integer[i + j + 1] += product.integer[i + j] / 10;
            product.integer[i + j] %= 10;
        }

    while (product.integer[product.integer.size() - 1] == 0) product.integer.pop_back();
...
HugeInteger< T > HugeInteger< T >::operator/(HugeInteger< T >& op2) {
    ...
    for (int k = quotient.integer.size() - 1; k >= 0; --k) {
        quotient.integer[k] = 10;
        do {
            --quotient.integer[k];
            buffer = op2 * quotient;
        } while (quotient.integer[k] > 0 && *this < buffer);
    }

    while (quotient.integer[quotient.integer.size() - 1] == 0) quotient.integer.pop_back();
